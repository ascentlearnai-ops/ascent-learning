...
import { generateAPSummary, generateAPQuiz, generateFlashcards } from '../services/aiService';
...
const APCenter: React.FC<APCenterProps> = ({ onExamReady }) => {
  ...
  const handleStudyUnit = async (unit: APUnit, index: number) => {
    ...
    try {
      ...
      const summary = await generateAPSummary(topicString);
      const quiz = await generateAPQuiz(topicString, 20);
